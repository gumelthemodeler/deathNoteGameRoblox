...
PrestigeAction.OnServerInvoke = function(player)
	...
	if currentPart >= 8 then
		...
		local ptsGain = 5 + (pLevel * 2) -- Matches the UI calculation
		...
		player:SetAttribute("PrestigePoints", (player:GetAttribute("PrestigePoints") or 0) + ptsGain)
		NotificationEvent:FireClient(player, "You have Prestiged! +" .. ptsGain .. " Prestige Points acquired!", "Success")
		return true
	else
